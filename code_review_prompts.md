...
|------|----------|----------|
| **Tier 1 — Covert/Operational** | Tool runs on target, stealth matters, detection = mission failure | malagent, loaders, implants, UDRL work, sleep masking, anything touching the CI/CD obfuscation pipeline output |
| **Tier 2 — Assessment & Exploitation** | Tool runs with authorization to find/exploit vulns, correctness and reliability matter most | aipostex, bloodhound-mcp, Go Snaffler port, Stowaway/dredge, NTLM relay tooling |
| **Tier 3 — Infrastructure & Pipeline** | Build systems, lab automation, support tooling | GitLab CI/CD pipeline, Ludus range configs, ThreatCheck wrapper, findings triage platform |
