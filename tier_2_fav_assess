...
Map the repo structure. Identify entrypoints, module/plugin architecture, CLI interface, output mechanisms. Understand what the tool claims to do — enumerate, exploit, extract, scan — and how the operator interacts with it.
Inventory dependencies. Pinning hygiene, lockfile presence. Flag dependencies that are unmaintained, abandoned, or have known bugs that would affect protocol/API interactions. Note anything that would make installation painful in a typical pentest environment (Kali, macOS, Windows, containers).
Run existing tests. Record pass/fail/skip. Baseline failures documented separately. Note any tests that require live target infrastructure to run and whether that's documented.
Identify the target surface. List every target technology, protocol, or API the tool interacts with (e.g., Ollama API, ChromaDB REST, MCP protocol, SMB/LDAP/Kerberos, Docker socket, LangChain configs). This inventory drives the rest of the review.
